...
set(nxv_WEB_DIR "${CMAKE_SOURCE_DIR}/web")
set(nxv_COI_WORKER "${nxv_WEB_DIR}/coi-serviceworker.min.js")
set(nxv_SHELL_TEMPLATE "${nxv_WEB_DIR}/shell_template.html.in")
set(nxv_INDEX_TEMPLATE "${nxv_WEB_DIR}/index_template.html.in")
...
if(EMSCRIPTEN)
  ...
  set(nxv_WASM_INITIAL_MEMORY "33554432" CACHE STRING
      "Initial WASM memory in bytes (default: 32MB)")
  set(nxv_WASM_PTHREAD_POOL_SIZE "4" CACHE STRING
      "Pthread pool size for WASM builds (default: 4)")
  set(nxv_WASM_ASYNCIFY_STACK_SIZE "65536" CACHE STRING
...
function(nxv_configure_wasm_target target)
  if(EMSCRIPTEN)
    ...
    set_property(GLOBAL APPEND PROPERTY nxv_WASM_TARGETS "${target}")
    set_property(GLOBAL PROPERTY nxv_WASM_TARGET_${target}_TITLE "${WASM_TITLE}")
    set_property(GLOBAL PROPERTY nxv_WASM_TARGET_${target}_DESCRIPTION "${WASM_DESCRIPTION}")
    set_property(GLOBAL PROPERTY nxv_WASM_TARGET_${target}_OUTPUT_NAME "${OUTPUT_NAME}")

    target_compile_definitions(${target} PRIVATE nxv_WASM_BUILD=1)
    ...
    target_link_options(${target} PRIVATE
      ...
      "-sUSE_PTHREADS=1"
      "-sPROXY_TO_PTHREAD=1"
      "-sPTHREAD_POOL_SIZE=${nxv_WASM_PTHREAD_POOL_SIZE}"
      # Enable asyncify for emscripten_sleep and async operations
      "-sASYNCIFY=1"
      "-sASYNCIFY_STACK_SIZE=${nxv_WASM_ASYNCIFY_STACK_SIZE}"
      # Memory configuration
      "-sALLOW_MEMORY_GROWTH=1"
      "-sINITIAL_MEMORY=${nxv_WASM_INITIAL_MEMORY}"
    ...
    if(EXISTS "${nxv_SHELL_TEMPLATE}")
      ...
      message(FATAL_ERROR "Shell template not found: ${nxv_SHELL_TEMPLATE}")
    endif()

    # Copy service worker to target build directory for standalone target builds
    if(EXISTS "${nxv_COI_WORKER}")
...
function(nxv_create_web_dist)
  ...
  get_property(WASM_TARGETS GLOBAL PROPERTY nxv_WASM_TARGETS)
  ...
  foreach(target ${WASM_TARGETS})
    get_property(TITLE GLOBAL PROPERTY nxv_WASM_TARGET_${target}_TITLE)
    get_property(DESCRIPTION GLOBAL PROPERTY nxv_WASM_TARGET_${target}_DESCRIPTION)
...
")
  ...
  if(EXISTS "${nxv_INDEX_TEMPLATE}")
    configure_file("${nxv_INDEX_TEMPLATE}" "${INDEX_OUTPUT}" @ONLY)
  else()
    message(WARNING "Index template not found: ${nxv_INDEX_TEMPLATE}")
  endif()
  ...
  foreach(target ${WASM_TARGETS})
    ...
    get_property(OUTPUT_NAME GLOBAL PROPERTY nxv_WASM_TARGET_${target}_OUTPUT_NAME)
